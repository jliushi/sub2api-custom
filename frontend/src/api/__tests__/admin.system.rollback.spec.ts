...
const longRequestConfig = { timeout: 15 * 60 * 1000 }
...
describe('admin system rollback API', () => {
  ...
  it('rollback posts the target version in the request body', async () => {
    ...
    expect(post).toHaveBeenCalledWith('/admin/system/rollback', { version: '0.1.146' }, longRequestConfig)
  ...
  it('rollback without a version posts no body (legacy backup rollback)', async () => {
    ...
    expect(post).toHaveBeenCalledWith('/admin/system/rollback', undefined, longRequestConfig)
  })
})
